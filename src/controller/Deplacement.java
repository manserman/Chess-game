...
public boolean faisPartie(ArrayList<Case> posibles, Case cse)// pour savoir si la case choisie peut être atteinte avec le pion choisit
...
{// second clic si la case choisie pour le deplacemnt est un déplacemnt légal
...
{if(cse.getPiece().getNom().equals("roi"))
	{JOptionPane.showMessageDialog(null,"Le joueur Blanc a gagné","Alert",JOptionPane.WARNING_MESSAGE);
	int1.dispose();
	}
// Doit faire un reset
...
move(Plateau.getPieceADeplacer().getPosition(), cse, p);// déplacement
	chrono.arreter();
...
private class Jouernoir extends Thread{// thread pour les déplacements du joueur noir
	@Override
	public void run()
	{tourblanc.stop();
	...
	ArrayList<Piece> pieces=Plateau.getnoirs();//les pièces noires restants sur le plateau etg qui peuvent bouger
	if(pieces.size()>0)// s'il reste plus d'une pièce
	{int index=new Random().nextInt()%pieces.size();// choix du pion à déplacer
	index=java.lang.Math.abs(index);
	boolean so=true;
	while(so)
	{
		{if(pieces.get(index)==null)// s'il n'y a aucune pièce à l'index index
		{so=true;
    ...
    Plateau.setPieceADeplacer(pieces.get(index));// on notifie au plateau la pièce à déplacer
	...
	if(cible.getPiece()!=null)// case d'une prise pour le joueur noir autrement dit la case cible est occupée par une pièce blanche
	{if(cse.getPiece().getNom().equals("roi"))
		{JOptionPane.showMessageDialog(null,"L'ordinateur a gagné","Alert",JOptionPane.WARNING_MESSAGE);  
		int1.dispose();}  
	...
	move(Plateau.getPieceADeplacer().getPosition(),cible,Plateau.getPieceADeplacer() );// déplacement
	...
	int i=0; while(!j2.getRestants().get(i).equals(Plateau.getPieceADeplacer()))
		i++;
	promouvoir(Plateau.getPieceADeplacer(),promotion,j2);
	j2.remplacer(promotion, i);
		}
	Plateau.setClick(0);
	// Nouveau
	chrono=new Chrono();
